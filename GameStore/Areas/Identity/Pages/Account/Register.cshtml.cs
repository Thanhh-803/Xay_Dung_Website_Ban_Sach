...
namespace GameStore.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private GameStoreUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<GameStoreUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(GameStoreUser)}'. " +
                    $"Ensure that '{nameof(GameStoreUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<GameStoreUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<GameStoreUser>)_userStore;
        }
    }
}
